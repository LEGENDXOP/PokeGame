...
fun FightModeScreen(modifier: Modifier) {
    ...
    Column(modifier = modifier.fillMaxSize(), horizontalAlignment = Alignment.CenterHorizontally) {
        if (!isFight){
            ...
            val enemyPokeMoves = enemyPokeFirst.moves.filter { it.levelLearnedAt <= enemyPokeFirst.level }
            ...
            var pokeBalls = pokeList.first().pokeBalls.first().takeIf { it.quantity > 0 }
            Image(
                painter = painterResource(R.drawable.mega_mewtwo_x), contentDescription = null,
                modifier = Modifier
                    .size(200.dp)
            ...
            val fightDetailText = "${myPokeFirst.name} (${myPokeFirst.level}) vs ${enemyPokeFirst.name} (${enemyPokeFirst.level})\n${myPokeFirst.name} has $myPokeHp HP\n${enemyPokeFirst.name} has $enemyPokeHp HP"
...
//            here we can add a button to run away and catch the pokemon
...
fun BattleButtons(listOfMoves: List<Move>, onClick: (Move) -> Unit) {
    FlowRow(maxItemsInEachRow = 2, modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceEvenly) {
